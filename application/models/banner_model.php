...
Class Banner_model extends CI_Model
{
	...
	function GetCategoryDeatails($category){
		$data['category'] =  $this->db->where('id', $category)->get('categories')->row();
		$data['subcategory'] =  $this->db->where('parent_id', $category)->where('company',0)->get('categories')->result();
		$sql = "select * from gc_categories  where parent_id = ".$category." and company = 1";
	...
	function banner_collection_NewProducts($only_active=false, $limit=5){
		$this->db->where('RecentProduct', 1);
		$banners	= $this->db->order_by('price', 'ASC')->get('products')->result();
		...
		if($only_active)
		{
			...
			foreach ($banners as $banner)
			{
				...
				if ($banner->StartDate == '0000-00-00')
				{
					$enable_test	= false;
					$enable			= '';
				}
				...
				{
					$eo			 	= explode('-', $banner->StartDate);
					$enable_test	= $eo[0].$eo[1].$eo[2];
					$enable			= $eo[1].'-'.$eo[2].'-'.$eo[0];
				}

				if ($banner->EndDate == '0000-00-00')
				{
					$disable_test	= false;
					$disable		= '';
				}
	...
	function banner_collection_PopularProducts(){
		$this->db->where('operator >=', 1);
		$this->db->where('SimPlanType !=', '');
		$banners	= $this->db->order_by('price', 'ASC')->get('products')->result();
	...
	function banner_collection_FilterProducts($theme, $category){
		$sql = "select * from gc_category_products as cp left join gc_products as p on cp.product_id = p.id where cp.category_id =$category 
		and PopularProduct = 1 and operator >= 1";
		$query = $this->db->query($sql);
