...
Class order_model extends CI_Model
{
	...
	function get_neworders(){
		...
		and d.ordertype_id =a.order_type and b.restaurant_manager = c.id and b.restaurant_manager='".$userdata['id']."' and a.status IN ('Order Placed', 'Assigned', 'Rejected') and a.ordered_on >='".$date."' order by a.ordered_on desc");
		if($sql->num_rows() > 0){
			$result	= $sql->result();
		}else{
			$result = 0;
	...
	function get_delpartnerorders(){
		...
		$sql = $this->db->query("SELECT a.*,d.order_type,d.ordertype_id,b.* FROM `orders` a, restaurant b, order_type d, admin c WHERE a.`restaurant_id` = b.restaurant_id 
		and d.ordertype_id =a.order_type and b.restaurant_manager = c.id and a.restaurant_manager_status = 'Accepted'  and a.order_type != 3 and a.ordered_on >= '".$date."' and a.delivery_partner != '123' order by a.ordered_on desc");
		if($sql->num_rows() > 0){
			$result	= $sql->result();
		}else{
			$result = 0;
	...
	function ChangeRestMangerStatus($status,$id){
		if($status == "1"){ 
			...
			$query1 = $this->db->query("SELECT `phone` FROM `customers` a,orders b where b.customer_id=a.id and b.id=".$id." ");
		     ...
		     $url =file("http://123.63.33.43/blank/sms/user/urlsmstemp.php?username=wolotech&pass=wolotech@654&senderid=EATSAP&dest_mobileno=" .$registatoin_ids[0] ."&tempid=52492&message=your+order+is+cancled&response=Y");
	       }
    ...
    function get_previousorders1($data){
		...
		and d.ordertype_id =a.order_type and b.restaurant_manager = c.id and b.restaurant_manager='".$userdata['id']."' and a.ordered_on >= '".$data['fromdate']."' and a.ordered_on <= '".$data['todate']."' order by ordered_on desc");
			...
			and d.ordertype_id =a.order_type and a.delivery_partner = c.id   and (a.ordered_on >= '".$data['fromdate']."' and a.ordered_on <= '".$data['todate']."') ".$where."
			 order by ordered_on desc");
		}
