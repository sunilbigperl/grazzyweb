...
Class order_model extends CI_Model
{
	...
	function get_previousorders($data){
		...
		if($this->auth->check_access('Restaurant manager')){
			...
			$sql = $this->db->query("SELECT a.*,d.order_type,d.ordertype_id,b.* FROM `orders` a, restaurant b, order_type d, admin c WHERE a.`restaurant_id` = b.restaurant_id 
			and d.ordertype_id =a.order_type and b.restaurant_manager = c.id and b.restaurant_manager = '".$userdata['id']."' and a.restaurant_manager_status = 'Accept' and (a.ordered_on >= '".$data['fromdate']."' and a.ordered_on <= '".$data['todate']."') order by ordered_on desc");
