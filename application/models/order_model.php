...
Class order_model extends CI_Model
{
	...
	function get_neworders(){
		...
		and d.ordertype_id =a.order_type and b.restaurant_manager = c.id and b.restaurant_manager='".$userdata['id']."' and a.status IN ('Order Placed', 'Assigned', 'Rejected','Picked Up') and a.ordered_on >='".$date."' order by a.ordered_on desc");
		if($sql->num_rows() > 0){
			$result	= $sql->result();
		}else{
			$result = 0;
	...
	function get_delpartnerorders(){
		...
		$sql = $this->db->query("SELECT a.*,d.order_type,d.ordertype_id,b.* FROM `orders` a, restaurant b, order_type d, admin c WHERE a.`restaurant_id` = b.restaurant_id 
		and d.ordertype_id =a.order_type and b.restaurant_manager = c.id and a.restaurant_manager_status = 'Accepted' and a.status='Picked UP' and a.order_type != 3 and a.ordered_on >= '".$date."' and a.delivery_partner != '123' order by a.ordered_on desc");
		if($sql->num_rows() > 0){
			$result	= $sql->result();
		}else{
			$result = 0;
