...
Class order_model extends CI_Model
{
	...
	function get_delpartnerorders(){
		...
		and d.ordertype_id =a.order_type and b.restaurant_manager = c.id and a.restaurant_manager_status = 'Accepted' and a.status!='Shipped' and a.status!='Rejected' and a.order_type != 3 and a.ordered_on >= '".$date."' and a.delivery_partner != '123' order by a.ordered_on desc");
		// echo "SELECT a.*,d.order_type,d.ordertype_id,b.* FROM `orders` a, restaurant b, order_type d, admin c WHERE a.`restaurant_id` = b.restaurant_id 
	...
	function get_previousorders($data){
		...
		and d.ordertype_id =a.order_type and b.restaurant_manager = c.id and b.restaurant_manager='".$userdata['id']."' and a.ordered_on >= '".$data['fromdate']."' and a.ordered_on < '".$data['todate']."' and a.status!='Payment pending' and a.status IN ('Delivered', 'Shipped','Rejected')  order by ordered_on desc");
