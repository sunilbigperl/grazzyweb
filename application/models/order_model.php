...
Class order_model extends CI_Model
{
	...
	function GetCustomerReview($id,$type){
		$sql = $this->db->query("select a.*,b.firstname from feedback a, customers b where a.feedbackfrom=b.id and a.feedbacktype='".$type."' and a.feedbackto='".$id."'");
		if($sql->num_rows() > 0){
	...
	function GetDelPartnerReview($id,$type){
	...
	function GetMenudetails($data){
		...
		$sql = $this->db->query("select a.*,b.*,c.restaurant_name from order_items a, restaurant_menu b,restaurant c  where a.menu_id=b.menu_id and b.restaurant_id=c.restaurant_id  and order_id='".$data['id']."'");
		if($sql->num_rows() > 0){
			$result	= $sql->result();
		}else{
			$result = 0;
