...
Class Customer_model extends CI_Model
{
    ...
    */
	...
	function GetRestSuggestions($all = false){
		...
		$sql = $this->db->query("select * from restaurant_suggest a, customers b where a.id=b.id ".$where."");
		if($sql->num_rows() > 0){
			$result['data']	= $sql->result();
		}else{
			$result =0;
	...
	function GetPitstopSuggestion($all = false){
		...
		$sql1 = $this->db->query("select * from pitstop_suggest a, customers b where a.id=b.id  ".$where."");
		if($sql1->num_rows() > 0){
