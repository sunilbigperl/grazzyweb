...
<div id="map" style="width: 1080px; height: 500px;"></div>
  ...
  <script type="text/javascript">
	...
	var icon = {
		url: "https://app.eatsapp.in/assets/img/Map1.png", // url
	...
	<tbody>
		...
		$GLOBALS['admin_folder'] = $this->config->item('admin_folder');
			...
			<tr class="gc_row">
				<td><?=$i;?></td>
				<td>
					<a href="#" data-toggle="modal" data-target="#orderdetails" onclick="showdetails('<?php echo site_url($this->config->item('admin_folder').'/orders/getMenuDetails');?>',<?=htmlspecialchars(json_encode($order));?>);" style="color: #2f2fd0;text-decoration:underline;"><?=$order->order_number;?></a>
				</td>
				<td>
					<?=$order->total_cost; ?>
				</td>
				...
				<td> 
					<?php if($order->ordertype_id == 3){ ?>
						<a href="#" data-toggle="modal" style="color: #2f2fd0;text-decoration:underline;"  data-target="#orderdetails" class="btn btn-primary btn-xs" onclick="showdetails('<?php echo site_url($this->config->item('admin_folder').'/orders/Review/2');?>',<?=htmlspecialchars(json_encode($order));?>);">Review customer</a>
					<?php }else{ 
